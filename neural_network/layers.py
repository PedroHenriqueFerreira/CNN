    ...
    def backward(self, output_gradient: list[Matrix]) -> list[Matrix]:
        ...
        for f in range(self.filters):
            ...
            for c in range(channels):
                ...
                input_gradient[c] += Matrix.convolve(output.pad(v_pad, h_pad), kernel, self.stride)
...
class Flatten(Layer):
    ...
    def output_shape(self) -> tuple[int, int]:
        ...
        return (channels * height * width, 1)
    
    def forward(self, input_value: list[Matrix]) -> Matrix:
        self.input_value = input_value
        
        values = Matrix.join(input_value).to_list()
        
        return Matrix(*self.output_shape()).fill(values)
    
    def backward(self, output_gradient: Matrix) -> list[Matrix]:
        assert self.input_shape is not None
        
        channels, height, width = self.input_shape
    
        input_gradient: list[Matrix] = []
    
        output_list = output_gradient.to_list()
    
        for i in range(channels):
            values = output_list[i * height * width : (i + 1) * height * width]
            
            input_gradient.append(Matrix(height, width).fill(values))
...
class Activation(Layer):
    ...
    def forward(self, input: Matrix | list[Matrix]) -> Matrix | list[Matrix]:
        self.input = input
        
        if isinstance(input, Matrix):
            return self.activation(input)
            
        return [self.activation(i) for i in input]
    
    def backward(self, output_gradient: Matrix | list[Matrix]) -> Matrix | list[Matrix]:
        if isinstance(output_gradient, Matrix) and isinstance(self.input, Matrix):
            return self.gradient(self.input) * output_gradient
        
        elif isinstance(output_gradient, list) and isinstance(self.input, list):
            return [self.gradient(self.input[i]) * output_gradient[i] for i in range(len(output_gradient))]
        
        raise TypeError('Invalid type for output_gradient or input')
